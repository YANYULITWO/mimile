package searchWholeTableServlet;
...
/**
 * Servlet implementation class SerchWholeTableServlet
 */
@WebServlet(description = "模糊搜索整个数据库", urlPatterns = { "/searchWholeTableServlet" })
public class SearchWholeTableServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchWholeTableServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		 BufferedReader read = request.getReader();
	       ...
	       String string =URLDecoder.decode(requestParam.getString("search"),"utf-8");
	       String[] a = string.split("\\s+");
	       String sql = String.format( "select id,name,img,price,zpmcc from %s where",DBNames.Table_SP);
	       String temp = null;
	       for(int i = 0; i<a.length; i++)
	       {
	    	   ...
	    	   if(temp ==null){
		     	...
		     	temp =  String.format(" wholeInfo like '%%%s%%'",a[i]);
				
					
	    	   }else{
	    		   temp = String.format(" and wholeInfo like '%%%s%%'",a[i]);
